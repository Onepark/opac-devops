...
def wait_for_available_instance(
    rds, state_machine_context: dict | None = None, db_instance_id: str | None = None
):
    ...
    waiter = botocore.waiter.create_waiter_with_client(
        "DBInstanceAvailable", _WAITER_MODEL, rds
    )
    ...
    logging.info(
        f"RDS instance ready: {identifier} (status={instance['DBInstanceStatus']})"
    )
...
def get_ephemeral_db_connection(rds_client, state_machine_context: dict):
    ...
    logging.info(
        f"Connecting to {params['host']}:{params['port']} database={params['database']} user={params['user']}"
    )
    try:
